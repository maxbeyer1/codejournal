...
export class SessionController {
  ...
  public startSession(): Session | undefined {
    ...
    if (this.currentSession && !this.currentSession.endTime) {
      vscode.window.showInformationMessage(
        "A CodeJournal session is already active."
      );
      return undefined;
    }
    ...
    Logger.info(
      `Session started with ID: ${this.currentSession.id}`,
      "SessionController"
    );
    Logger.info(
  ...
  public async stopSession(): Promise<Session | undefined> {
    ...
    if (!this.currentSession || this.currentSession.endTime) {
      vscode.window.showInformationMessage(
        "No active CodeJournal session to stop."
      );
      return undefined;
    }
    ...
    const sessionChanges =
      this.changeTracker?.getChangesBySession(this.currentSession.id) || [];

    // Log session summary for debugging
    Logger.info(
      `Session with ID ${this.currentSession.id} stopped`,
      "SessionController"
    );
    Logger.debug(
      `Start time: ${this.currentSession.startTime}`,
    ...
    Logger.debug(
      `End time: ${this.currentSession.endTime}`,
      "SessionController"
    );
    Logger.debug(
      `Duration: ${this.calculateDuration(this.currentSession)} minutes`,
      "SessionController"
    );
    Logger.debug(
      `Total changes: ${sessionChanges.length}`,
      "SessionController"
    );
    ...
    if (sessionChanges.length > 0) {
      ...
      for (const [type, changes] of Object.entries(changesByType)) {
        Logger.debug(
          `- ${type}: ${changes.length} changes`,
          "SessionController"
        );
      }

      Logger.debug("Change IDs:", "SessionController");
      sessionChanges.forEach(
        (change: { id: string; type: string; filePath: string }) => {
          Logger.debug(
            `- ${change.id} (${change.type}: ${change.filePath})`,
            "SessionController"
          );
        }
      );
      ...
      try {
        Logger.info(
          "Generating summary for session changes",
          "SessionController"
        );
        ...
        if (result.summary) {
          const formattedSummary = this.summarizer.formatSummaryForConsole(
            result.summary
          );
          ...
          try {
            ...
            Logger.info(
              "Session summary added to journal file",
              "SessionController"
            );
            ...
            Logger.error(
              "Failed to save summary to journal file",
              "SessionController",
              error
            );
            vscode.window.showErrorMessage(
              "CodeJournal: Failed to save summary to journal file."
            );
          }
          ...
          if (result.error.retryable) {
            ...
            vscode.window
              .showErrorMessage(retryMessage, "Retry")
              .then((selection) => {
                if (selection === "Retry") {
          ...
          Logger.error(
            "Failed to generate summary for unknown reason",
            "SessionController"
          );
          vscode.window.showErrorMessage(
            "CodeJournal: Failed to generate summary for unknown reason."
          );
        }
      } catch (error) {
    ...
    vscode.window.showInformationMessage(
      `CodeJournal session stopped. ${sessionChanges.length} changes recorded.`
    );
  ...
  private updateStatusBar(): void {
    if (this.isSummarizing) {
      this.statusBarItem.text =
        "$(loading~spin) CodeJournal: Generating Summary";
      this.statusBarItem.tooltip =
        "CodeJournal is generating a summary of the session";
   ...
   */
  private async retryGenerateSummary(
    session: Session,
    sessionChanges: any[]
  ): Promise<void> {
    ...
    try {
      ...
      if (result.summary) {
        ...
        try {
          await this.journalManager.addToJournal(result.summary);
          Logger.info(
            "Session summary added to journal file after retry",
            "SessionController"
          );
          vscode.window.showInformationMessage(
            "CodeJournal: Summary generated successfully after retry."
          );
          ...
          Logger.error(
            "Failed to save summary to journal file after retry",
            "SessionController",
            error
          );
          vscode.window.showErrorMessage(
        ...
        Logger.error(
          "Retry summary generation failed",
          "SessionController",
          result.error
        );
        // Don't show another retry option to avoid infinite loops
        vscode.window.showErrorMessage(
          `CodeJournal: Retry failed - ${result.error.message}`
        );
      }
    } catch (error) {
      Logger.error(
        "Error during retry summary generation",
        "SessionController",
        error
      );
